         ...
         <div class="d-sm-flex align-items-center justify-content-between">
            <div class="sec-intro">
               <span class="sub-title fadeInUp"> <svg xmlns="http://www.w3.org/2000/svg" width="30" height="18"
                  ...
                  </svg> {{ __($portfolioHeader?->subtitle ?? 'Our Exports') }}</span>
               <h2 class="" data-cms-field="title">{{ __($portfolioHeader?->title ?? 'Premium Agricultural Products') }}</h2>
            </div>
            ...
            <div class="col-lg-12">
               <div class="portfolio-carousel swiper">
                  <div class="swiper-wrapper">
                     @forelse($portfolioItems ?? collect() as $pItem)
                     <div class="swiper-slide">
                        <div class="portfolio-card">
                           <div class="portfolio-img">
                              <a href="{{ route('client.project', $pItem->slug) }}">
                                 <img class="img-fluid" src="{{ $pItem->getFirstMediaUrl('cover') ?: asset('client/assets/images/10475.jpg') }}" alt="{{ $pItem->title }}">
                              </a>
                           </div>
                           <div class="portfolio-info">
                              <span>{{ __($pItem->subtitle ?? '') }}</span>
                              <h3 class="display-4"><a href="{{ route('client.project', $pItem->slug) }}">{{ __($pItem->title) }}</a></h3>
                           </div>
                        </div>
                     </div>
                     @empty
                     <div class="swiper-slide">
                        <div class="portfolio-card">
                           <div class="portfolio-img">
                              <a href="{{ route('client.projects') }}"><img class="img-fluid" src="{{ asset('client/assets/images/10475.jpg') }}" alt=""></a>
                           </div>
                           <div class="portfolio-info">
                              <span>{{ __('Vegetables') }}</span>
                              <h3 class="display-4"><a href="{{ route('client.projects') }}">{{ __('Fresh Vegetables') }}</a></h3>
                           </div>
                        </div>
                     </div>
                     @endforelse
